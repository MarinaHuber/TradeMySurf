...
public class StringPickerPopover: AbstractPopover {
    ...
    private(set) var clearButton: ButtonParameterType = (title: "test test".localized, font: nil, color: nil, action: nil)
...
//    }
    ...
    /// - Returns: Self
   public func setButton(button: inout ButtonParameterType, title: String? = nil, font: UIFont? = nil, color: UIColor? = nil, action: ActionHandlerType?) -> Self {
        if let title = title {
            button.title = title
        }
        if let font = font {
