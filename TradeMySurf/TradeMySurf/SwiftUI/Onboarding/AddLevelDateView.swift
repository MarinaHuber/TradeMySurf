    ...
    var body: some View {
        VStack(spacing: 0) {
            ArrowPopoverView(navigateToNext: $navigateToNext)
                .id(resetTrigger) // Force reset if triggered
        }
        ...
        // Monitor state change of navigateToNext
        .onChange(of: navigateToNext) { _ , next in
            if next {
                DispatchQueue.main.async {
                    resetState() // Trigger reset before the modal disappears
                }
            }
        }
    }
...
struct PickerPopoverView: View {
    ...
    @State private var levels = [
        Level.beginner.rawValue,
        Level.beginnerIntermediate.rawValue,
        Level.intermediate.rawValue,
        Level.advanced.rawValue,
        Level.areals.rawValue,
        Level.longboarding.rawValue
    ]
